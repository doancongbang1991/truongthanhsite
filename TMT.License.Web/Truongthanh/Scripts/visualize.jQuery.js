...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			colors: ['#47a1c6','#adbf69','#eccb4a','#e0564b','#7c7e7d','#ec8a71','#26a4ed','#f45a90','#e9e744'],
					...
					if(o.parseDirection == 'x'){
						...
						for(var i=0; i<cols; i++){
							...
							self.find('tr:gt(0)').each(function(){
								dataGroups[i].points.push( $(this).find('td').eq(i).text().replace ( /[^\d.]/g, '' )*1 );
							});
						};
					}
					return dataGroups;
			...
			pie: function(){	
				...
				$.each(memberTotals, function(i){
			        ...
			        // draw labels
			       	...
			       	var distance = o.pieLabelPos == 'inside' ? radius/1.3 : radius +  radius / 5;
			        var labelx = Math.round(centerx + Math.sin(sliceMiddle * Math.PI * 2) * (distance) - 10);
			        var labely = Math.round(centery - Math.cos(sliceMiddle * Math.PI * 2) * (distance));
					
					ctx.font="14px Georgia bold";
					ctx.fillStyle = "white";
					//ctx.fillText((fraction*100).toFixed(1) + '%',labelx,labely)
					ctx.fillText(Math.round(fraction*100) + '%',labelx,labely)
		...
		canvasNode.setAttribute('id','canvasGraphic');
