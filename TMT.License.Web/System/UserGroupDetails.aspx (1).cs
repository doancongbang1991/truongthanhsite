...
namespace TMT.License.Web.TSSystem
{
    public partial class UserGroupDetails : System.Web.UI.Page
    {
        ...
        {
            Response.Redirect(UserCommon.FormatPageWithParameter(UserCommon.TSSystem_UserGroupManager));
        }
        protected void btRefresh_Click(object sender, DirectEventArgs e)
        {
            int UGRPID = UserCommon.ToInt(this.hiID.Value);
            if (UGRPID > 0)
                Response.Redirect(UserCommon.FormatPageWithParameter(UserCommon.TSSystem_UserGroupDetails));
            else
                Response.Redirect(UserCommon.TSSystem_UserGroupDetails);
        }
        #endregion
